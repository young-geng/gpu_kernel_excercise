...
def delayed_run_scoped(*args, **kwargs):
    def delayed():
        return pl.run_scoped(*args, **kwargs)
    return delayed
...
def pallas_matmul(
    ...
    @partial(
        ...
        scratch_shapes=dict(
            ...
            smem_a_barrier_ref=plgpu.Barrier(num_barriers=num_buffers),
            smem_b_barrier_ref=plgpu.Barrier(num_barriers=num_buffers),
            tc_barrier_ref=plgpu.Barrier(num_barriers=num_buffers),
        ),
    ...
    def matmul_kernel(in_refs, o_ref, smem_a_ref, smem_b_ref, smem_acc_ref, smem_a_barrier_ref, smem_b_barrier_ref, tc_barrier_ref):
        a_ref, b_ref = in_refs
        ...
        @pl.when(thread_id == 1)
        @partial(delayed_run_scoped, acc_ref=plgpu.ACC((block_size[0], block_size[2]), jnp.float32))
        def mma_thread(acc_ref):
            for k in range(x.shape[1] // block_size[1]):
                slot = k % num_buffers
                ...
                if k >= 1:
                    completed_slot = (k - 1) % num_buffers
                    plgpu.barrier_arrive(tc_barrier_ref.at[completed_slot])
