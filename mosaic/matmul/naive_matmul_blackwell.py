...
def pallas_matmul(
    ...
    @partial(
        ...
        scratch_shapes=dict(
            ...
            smem_b_ref=plgpu.SMEM((block_size[1], block_size[2]), x.dtype, transforms=smem_transforms),
            ...
            smem_barrier_ref=plgpu.Barrier(orders_tensor_core=False, num_barriers=2),
            tc_barrier_ref=plgpu.Barrier(orders_tensor_core=True, num_barriers=1),
        ),
        grid=(x.shape[0] // block_size[0], y.shape[1] // block_size[2]),
        grid_names=("m", "n"),
    )
    def matmul_kernel(in_refs, o_ref, smem_a_ref, smem_b_ref, tmem_acc_ref, smem_acc_ref, smem_barrier_ref, tc_barrier_ref):
        a_ref, b_ref = in_refs
        ...
        for k in range(x.shape[1] // block_size[1]):
            ...
            plgpu.copy_gmem_to_smem(a_ref_tile, smem_a_ref, smem_barrier_ref.at[0])
            plgpu.copy_gmem_to_smem(b_ref_tile, smem_b_ref, smem_barrier_ref.at[1])
            plgpu.barrier_wait(smem_barrier_ref.at[0])
            plgpu.barrier_wait(smem_barrier_ref.at[1])
            plgpu.tcgen05_mma(tmem_acc_ref, smem_a_ref, smem_b_ref, tc_barrier_ref, accumulate=k > 0)
            plgpu.barrier_wait(tc_barrier_ref)
