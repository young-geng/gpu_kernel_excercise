...
def pallas_matmul(
    ...
    @partial(
        ...
        scratch_shapes=dict(
            ...
            smem_a_barrier_ref=plgpu.Barrier(orders_tensor_core=False, num_barriers=2),
            smem_b_barrier_ref=plgpu.Barrier(orders_tensor_core=False, num_barriers=2),
            tc_barrier_ref=plgpu.Barrier(orders_tensor_core=True, num_barriers=2),
    ...
    def matmul_kernel(in_refs, o_ref, smem_a_ref, smem_b_ref, tmem_acc_ref, smem_acc_ref, smem_a_barrier_ref, smem_b_barrier_ref, tc_barrier_ref, tc_done_barrier_ref):
        ...
        def load_data_thread():
            for k in range(x.shape[1] // block_size[1]):
                ...
                if k > 1:
                    plgpu.barrier_wait(tc_barrier_ref.at[buffer_id])

                plgpu.copy_gmem_to_smem(
                    a_ref_tile,
                    smem_a_ref.at[buffer_id, ...],
                    smem_a_barrier_ref.at[buffer_id]
                )
                plgpu.copy_gmem_to_smem(
                    b_ref_tile,
                    smem_b_ref.at[buffer_id, ...],
                    smem_b_barrier_ref.at[buffer_id]
                )
